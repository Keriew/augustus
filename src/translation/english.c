...
static translation_string all_strings[] = {
    ...
    {TR_CONFIG_LANGUAGE_LABEL_TOOLTIP, "Select a language other than the default Caesar III installation. To add a custom or modded language, create a folder in the Caesar III installation directory, copy the language files (c3.eng, c3_map.eng, c3_map_mm.eng, andc3_mm.eng) into it, then select the folder from the Language list andclick Apply."},
    {TR_CONFIG_LANGUAGE_DEFAULT, "(default)"},
    {TR_CONFIG_DEFAULT_PLAYER_NAME, "Default player name:"},
    {TR_CONFIG_GAME_SPEED, "Game speed:"},
    {TR_CONFIG_GAME_SPEED_TOOLTIP, "Set the in-game speed (can also be set via the UI during a game)"},
    ...
    {TR_CONFIG_RANDOMISE_MUSIC_TOOLTIP, "Play randomly any of the \"Rome1\" to \"Rome5\" tracks, with no loop. Intended to be used with custom music tracks."},
    ...
    {TR_CONFIG_SHOW_WATER_STRUCTURE_RANGE_TOOLTIP, "When building reservoirs, fountains and wells : displays the range of these buildings when placing them, and also the range of fountains and wells already built."},
    {TR_CONFIG_SHOW_WATER_STRUCTURE_RANGE_HOUSES, "When building houses"},
    {TR_CONFIG_SHOW_WATER_STRUCTURE_RANGE_HOUSES_TOOLTIP, "When building houses: displays the range of fountains and wells already built when placing houses."},
    {TR_CONFIG_SHOW_MARKET_RANGE, "Show range when building markets"},
    {TR_CONFIG_SHOW_MARKET_RANGE_TOOLTIP, "Shows how far a market lady will go to fetch goods. The range is drawn only if the \"Enable market range\" option is activated (City Management tab, Storage and Markets section)."},
    ...
    {TR_CONFIG_UI_SHOW_GRID_TOOLTIP, "Display a construction grid on the whole map."},
    ...
    {TR_CONFIG_RETIRE_AT_60_TOOLTIP, "Citizens retire at age 60 instead of 50, but this option has no effect when \"Fixed worker pool - 45% of plebeian population\" is enabled."},
   ...
   {TR_CONFIG_NO_SUPPLIER_DISTRIBUTION_TOOLTIP, "Market buyers will not distribute goods (e.g. wine) to houses on their way to storage buildings. This prevents plebeian houses from accidentally upgrading into patrician villas."},
    ...
    {TR_CONFIG_TOWER_SENTRIES_GO_OFFROAD_TOOLTIP, "Sentries can reach their assigned tower off-road, so a road connection between the Barracks and the Tower is no longer required, although the Tower must still have road access to receive workers."},
    {TR_CONFIG_FARMS_DELIVER_CLOSE, "Farms and wharves deliver only to nearby granaries"},
    {TR_CONFIG_FARMS_DELIVER_CLOSE_TOOLTIP, "Farms deliver food only to nearby Granaries, reducing delivery time. Maximum distance is 64 tiles in a straight line."},
    {TR_CONFIG_DELIVER_ONLY_TO_ACCEPTING_GRANARIES, "Food isn't delivered to getting granaries"},
    {TR_CONFIG_DELIVER_ONLY_TO_ACCEPTING_GRANARIES_TOOLTIP, "Enables better control over farm and wharf cart pushers behaviour by preventing them from taking their harvest or catch to a granary that is \"Getting\" that food type."},
    {TR_CONFIG_ALL_HOUSES_MERGE, "All houses merge"},
    {TR_CONFIG_ALL_HOUSES_MERGE_TOOLTIP, "Force houses in a 2x2 area that are of the same quality, such as \"Small Tent\", \"Large Casa\", or \"Small Insulae\", to merge into the 2x2 variant. This removes the random element assigned to map tiles to determine if homes will merge on that tile."},
    ...
    {TR_CONFIG_ROAMERS_DONT_SKIP_CORNERS_TOOLTIP, "If disabled, roaming walkers sometimes cut corners, and do not trigger their passage for buildings on the outer edge of the corner."},
    {TR_CONFIG_SHOW_MAX_POSSIBLE_PROSPERITY, "Display max attainable prosperity rating with current housing"},
    {TR_CONFIG_SHOW_MAX_POSSIBLE_PROSPERITY_TOOLTIP, "Shows the value in the Ratings Advisor, Prosperity tab"},
    {TR_CONFIG_AUTO_KILL_ANIMALS, "Citizens will automatically kill harmless animals"},
    {TR_CONFIG_AUTO_KILL_ANIMALS_TOOLTIP, "If disabled, peaceful animals (sheep and zebras) are not killed on sight, unless you send a legion."},
    {TR_CONFIG_SHOW_ROAMING_PATH, "Preview paths traveled by roaming walkers"},
    {TR_CONFIG_SHOW_ROAMING_PATH_TOOLTIP, "Displays walker roaming paths when viewing overlays or selecting a building: blue marks the route, green the spawn point, red the despawn point, and purple both on the same tile."},
    {TR_CONFIG_DRAW_CLOUD_SHADOWS, "Draw cloud shadows"},
    {TR_CONFIG_DRAW_CLOUD_SHADOWS_TOOLTIP, "Simulates the clouds casted shadows, slowly moving above the city."},
    ...
    {TR_CONFIG_DIGIT_SEPARATOR_TOOLTIP, "Displays thousand separators for large numbers."},
    ...
    {TR_CONFIG_SHOW_DESIRABILITY_RANGE_TOOLTIP, "Displays negative (red) or positive (green) layers around the building when placing it."},
    {TR_CONFIG_SHOW_DESIRABILITY_RANGE_ALL, "Show desirability when building any structures"},
    {TR_CONFIG_SHOW_DESIRABILITY_RANGE_ALL_TOOLTIP, "Displays negative (red) or positive (green) layers around the building when placing it."},
    ...
    {TR_CONFIG_DRAW_ASCLEPIUS_TOOLTIP, "Replaces the anachronic cross icon by the mythological symbol of medicine."},
    ...
    {TR_CONFIG_WAREHOUSES_GRANARIES_OVER_ROAD_PLACEMENT_TOOLTIP, "The loading bay of warehouses can be placed over a road corner, and granaries can be built directly over an existing road or intersection."},
    {TR_CONFIG_HIGHLIGHT_SELECTED_BUILDING, "Highlight selected building"},
    {TR_CONFIG_HIGHLIGHT_SELECTED_BUILDING_TOOLTIP, "Building selected with a mouse click is colored in blue, and all figures associated with it are highlighted in green for easier tracking."},
    ...
    {TR_CONFIG_MAX_AUTOSAVE_SLOTS_TOOLTIP, "Defines the maximum number of yearly autosaves to be stored, before the oldest one is overwritten."},
    ...
    {TR_CONFIG_CARAVANS_MOVE_OFF_ROAD_TOOLTIP, "For players using the \"forced walkers\" exploit, it may be better for land traders and ox carts to not follow the road network. Enabling this option is not advised for regular gameplay."},
    ...
    {TR_CONFIG_STORAGE_STEP_4_TOOLTIP, "Set the control of space allocation by multiples of 4 in these buildings. A left click increases the value by 4, a right click decreases it by 4."},
    {TR_CONFIG_MOVE_SAVINGS_TO_THE_RIGHT, "Original top bar items order"},
    {TR_CONFIG_MOVE_SAVINGS_TO_THE_RIGHT_TOOLTIP, "Augustus menu bar shows by default new items, like the private savings at the right of the city funds. Enabling this places the new items at the end of the menu bar, therefore keeping the order of the original items."},
    ...
    {TR_CONFIG_PATRICIAN_DEVOLUTION_FIX_TOOLTIP, "If enabled, Patrician houses from \"Medium Villa\" and above devolve correctly to the rank below, and not in several Insulae."},
    ...
    {TR_CONFIG_DEFAULT_GAME_SPEED_TOOLTIP, "Set the speed at which a game will always start, and revert after some in-game events (e.g. invasions)"},
    ...
    {TR_CONFIG_SHOW_CUSTOM_VARIABLES_TOOLTIP, "Effects of special events are displayed in a banner in the top-left corner of the city view (custom scenarios only)."},
    ...
    {TR_CONFIG_ENABLE_GETTING_WHILE_STOCKPILED_TOOLTIP, "Cart pushers from \"Getting\" buildings can fetch the designated resource, even if it is currently stockpiled by order of the Trade advisor."},
    {TR_CONFIG_ENABLE_PAVED_ROADS_NEAR_GRANARIES, "Pave roads next to granaries"},
    {TR_CONFIG_ENABLE_PAVED_ROADS_NEAR_GRANARIES_TOOLTIP, "Road tiles connecting the 4 entries of a granary will be paved by default (otherwise they stay as dirt road tiles, even if the whole road is paved)."},
    ...
    {TR_CONFIG_ADVANCED_CART_DEPOT_OPTIONS_TOOLTIP, "If enabled, cart depot orders also display storage buildings not accepting the resource currently handled by the cart depot."},
    ...
    {TR_CONFIG_ENABLE_MARKET_RANGE_TOOLTIP, "If enabled, market ladies will have a limit to how far they will go to fetch goods. The range can be drawn if the \'Show range when building markets\' option is also enabled."},
    ...
    {TR_CONFIG_UI_CV_CURSOR_SHADOW_TOOLTIP, "The tile pointed by the cursor is slightly darkened."},
    ...
    {TR_CONFIG_GP_CH_HOUSING_PRE_MERGE_VACANT_LOTS_TOOLTIP, "If enabled, prevents houses of the same quality from merging \'on the wrong side\', leaving 1x1 gaps."},
    ...
    {TR_CONFIG_ENABLE_BUILD_MENU_SHORTCUTS_TOOLTIP, "Items in construction lists can be selected with numerical keys 0-9 when the build menu is opened (note: hotkeys defined for numerical keys still have their proper behaviour outside of the build menu)."},
    ...
    {TR_CONFIG_GRANARY_DEFAULT_TO_PASS_ALL_WALKERS_TOOLTIP, "The Granary includes a built-in roadblock that can be configured to block all walkers by default."},
    {TR_CONFIG_WAREHOUSE_DEFAULT_TO_PASS_ALL_WALKERS, "Warehouses do not allow walkers by default"},
    {TR_CONFIG_WAREHOUSE_DEFAULT_TO_PASS_ALL_WALKERS_TOOLTIP, "The Warehouse includes a built-in roadblock that can be configured to block all walkers by default."},
    ...
    {TR_CONFIG_SHOW_SHORELINE_DESIRABILITY_TOOLTIP, "The Desirability overlay displays colored tiles for this option, just like it does for buildings."},
    {TR_CONFIG_SHOW_ELEVATION_DESIRABILITY, "Show elevation desirability in overlay"},
    {TR_CONFIG_SHOW_ELEVATION_DESIRABILITY_TOOLTIP, "The Desirability overlay displays colored tiles for this option, just like it does for buildings."},
