...
static void button_close_military_new_sidebar(int param1, int param2);
...
static image_button buttons_title_close[] = {
    {123, 4, 39, 26, IB_NORMAL, GROUP_OK_CANCEL_SCROLL_BUTTONS, 4,
        button_close_military_new_sidebar, button_none, 0, 0, 1},
...
int widget_new_sidebar_military_get_standard_image(int legion_id)
{
    switch (legion_id) {
...
int widget_new_sidebar_military_get_legion_name_group(int legion_id)
{
    return legion_id <= 10 ? 138 : 10000;
}

int widget_new_sidebar_military_get_legion_name_id(int legion_id)
{
    if (legion_id <= 10) {
...
int widget_new_sidebar_military_handle_input(const mouse *m)
{
    ...
    return new_sidebar_extra_handle_mouse(m);
}
...
int widget_new_sidebar_military_get_tooltip_text(tooltip_context *c)
{
...
int widget_new_sidebar_military_enter(int formation_id)
{
    clear_focus_buttons();
    ...
    if (data.city_view_was_collapsed) {
        ...
        new_sidebar_slide(SLIDE_DIRECTION_IN, widget_new_sidebar_city_draw_background, draw_sliding, slide_in_finished);
    } else {
        slide_in_finished();
    }
    return 1;
...
int widget_new_sidebar_military_exit(void)
{
    clear_focus_buttons();
    ...
    if (data.city_view_was_collapsed) {
        city_view_toggle_sidebar();
        new_sidebar_slide(SLIDE_DIRECTION_OUT, widget_new_sidebar_city_draw_background, draw_sliding, slide_out_finished);
    } else {
        slide_out_finished();
    }
    return 1;
...
static void button_close_military_new_sidebar(int param1, int param2)
{
    widget_new_sidebar_military_exit();
}
