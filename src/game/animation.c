...
void game_animation_update(void)
{
    ...
    for (int i = 0; i < MAX_ANIM_TIMERS; i++) {
        if (now_millis - timers[i].last_update >= delay_millis && (window_is(WINDOW_CITY) || window_is(WINDOW_EMPIRE) || window_is(WINDOW_EDITOR_EMPIRE) || window_is(WINDOW_TRADE_OPENED))) {
