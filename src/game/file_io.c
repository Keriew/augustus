...
static void get_version_data(savegame_version_data *version_data, savegame_version version)
{
    ...
    if (version > SAVE_GAME_LAST_NO_EVENTS) {
        version_data->piece_sizes.scenario_events = PIECE_SIZE_DYNAMIC;
        version_data->features.scenario_events = 1;
        version_data->piece_sizes.scenario_conditions = PIECE_SIZE_DYNAMIC;
        version_data->features.scenario_conditions = 1;
        version_data->piece_sizes.scenario_actions = PIECE_SIZE_DYNAMIC;
        version_data->features.scenario_actions = 1;
    } else {
        version_data->piece_sizes.scenario_events = 1;
        version_data->features.scenario_events = 0;
        version_data->piece_sizes.scenario_conditions = 1;
        version_data->features.scenario_conditions = 0;
        version_data->piece_sizes.scenario_actions = 1;
...
static void init_savegame_data(savegame_version version)
{
    ...
    if (version_data.features.scenario_events) {
        state->scenario_events = create_savegame_piece(version_data.piece_sizes.scenario_events, 0);
    }
    if (version_data.features.scenario_conditions) {
        state->scenario_conditions = create_savegame_piece(version_data.piece_sizes.scenario_conditions, 0);
    }
    if (version_data.features.scenario_actions) {
        state->scenario_actions = create_savegame_piece(version_data.piece_sizes.scenario_actions, 0);
    }
