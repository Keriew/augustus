#include "core/log.h"

#include "SDL.h"

#include <stdio.h>
#include <string.h>

#define MSG_SIZE 1000
enum {
    LOG_HISTORY_MSG_LEN = 256,
    LOG_HISTORY_CAPACITY = 40
}; 

static char log_buffer[MSG_SIZE];

static unsigned int log_history_index = 0;
static char log_history_buf[LOG_HISTORY_CAPACITY][LOG_HISTORY_MSG_LEN];

static const char *build_message(const char *msg, const char *param_str, int param_int)
{
    int index = 0;
    index += snprintf(&log_buffer[index], MSG_SIZE - index, "%s", msg);
    if (param_str) {
        index += snprintf(&log_buffer[index], MSG_SIZE - index, "  %s", param_str);
    }
    if (param_int) {
        index += snprintf(&log_buffer[index], MSG_SIZE - index, "  %d", param_int);
    }

    strncpy(log_history_buf[log_history_index], log_buffer, LOG_HISTORY_MSG_LEN);
    log_history_index++;
    log_history_index %= LOG_HISTORY_CAPACITY;

    return log_buffer;
}

void log_info(const char *msg, const char *param_str, int param_int)
{
    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "%s", build_message(msg, param_str, param_int));
}

void log_error(const char *msg, const char *param_str, int param_int)
{
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "%s", build_message(msg, param_str, param_int));
}

const char *log_history_get_line(unsigned int line) {
    return log_history_buf[(log_history_index + line) % LOG_HISTORY_CAPACITY];
}

void log_history_clear() {
    int i = 0;
    for (; i < LOG_HISTORY_CAPACITY; i++)
        memset(log_history_buf[i], 0, LOG_HISTORY_MSG_LEN);
    log_history_index = 0;
}

unsigned int log_history_capacity(void) {
    return LOG_HISTORY_CAPACITY;
}
