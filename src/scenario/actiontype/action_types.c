...
void scenario_action_init(scenario_action_t *action)
{
    switch (action->type) {
        ...
        case ACTION_TYPE_ADJUST_FAVOR:
            scenario_event_action_favor_init(action);
            break;
        case ACTION_TYPE_ADJUST_MONEY:
            scenario_event_action_money_init(action);
            break;
        case ACTION_TYPE_ADJUST_ROME_WAGES:
            scenario_event_action_rome_wages_init(action);
            break;
        case ACTION_TYPE_ADJUST_SAVINGS:
            scenario_event_action_savings_init(action);
            break;
        case ACTION_TYPE_CHANGE_ALLOWED_BUILDINGS:
            scenario_event_action_change_allowed_buildings_init(action);
            break;
        case ACTION_TYPE_CHANGE_RESOURCE_PRODUCED:
            scenario_event_action_change_resource_produced_init(action);
            break;
        case ACTION_TYPE_EMPIRE_MAP_CONVERT_FUTURE_TRADE_CITY:
            scenario_event_action_empire_map_convert_future_trade_city_init(action);
            break;
        case ACTION_TYPE_GLADIATOR_REVOLT:
            scenario_event_action_gladiator_revolt_init(action);
            break;
        case ACTION_TYPE_REQUEST_IMMEDIATELY_START:
            scenario_event_action_request_immediately_start_init(action);
            break;
        case ACTION_TYPE_SEND_STANDARD_MESSAGE:
            scenario_event_action_send_standard_message_init(action);
            break;
        case ACTION_TYPE_TRADE_ADJUST_PRICE:
            scenario_event_action_trade_price_init(action);
            break;
        case ACTION_TYPE_TRADE_ADJUST_ROUTE_AMOUNT:
            scenario_event_action_trade_route_amount_init(action);
            break;
        case ACTION_TYPE_TRADE_PROBLEM_LAND:
            scenario_event_action_trade_problem_land_init(action);
            break;
        case ACTION_TYPE_TRADE_PROBLEM_SEA:
            scenario_event_action_trade_problem_sea_init(action);
            break;
        case ACTION_TYPE_TRADE_SET_PRICE:
            scenario_event_action_trade_price_set_init(action);
            break;
        default:
            break;
    }
...
int scenario_action_execute(scenario_action_t *action)
{
    switch (action->type) {
        ...
        case ACTION_TYPE_ADJUST_FAVOR: return scenario_event_action_favor_execute(action);
        case ACTION_TYPE_ADJUST_ROME_WAGES: return scenario_event_action_rome_wages_execute(action);
        case ACTION_TYPE_ADJUST_MONEY: return scenario_event_action_money_execute(action);
        case ACTION_TYPE_ADJUST_SAVINGS: return scenario_event_action_savings_execute(action);
        ...
        case ACTION_TYPE_TRADE_ADJUST_PRICE: return scenario_event_action_trade_price_execute(action);
        case ACTION_TYPE_TRADE_ADJUST_ROUTE_AMOUNT: return scenario_event_action_trade_route_amount_execute(action);
        case ACTION_TYPE_TRADE_PROBLEM_LAND: return scenario_event_action_trade_problem_land_execute(action);
        case ACTION_TYPE_TRADE_PROBLEM_SEA: return scenario_event_action_trade_problem_sea_execute(action);
