...
if ("${env:COMPILER}" -eq "msvc") {
    ...
    if ((Test-Path "$SDL_DIR\SDL${SDL_MAJOR_VERSION}.dll") -and (Test-Path "$SDL_MIXER_DIR\SDL${SDL_MAJOR_VERSION}_mixer.dll")) {
        ...
        cmake -G "Ninja" -A ARM64 -DSDL_LIBC=ON -DCMAKE_BUILD_TYPE=Release ..
