...
int custom_messages_get_id_by_uid(const uint8_t *message_uid, int *message_id)
{
...
custom_message_t *custom_messages_create_blank(void)
{
    ...
    array_new_item(custom_messages, 0, entry);
    if (!entry) {
        return 0;
    }
    entry->in_use = 1;
...
{
    int duplicate_message_id = 0;
    int found = custom_messages_get_id_by_uid(uid_text, &duplicate_message_id);
    if (found) {
        return 0;
    }
...
void custom_messages_save_state(buffer *buffer)
{
    ...
    custom_message_t *entry;
    array_foreach(custom_messages, entry) {
        buffer_write_i32(buffer, entry->linked_uid->id);
...
{
    // file_has_extension(filename, "mp3")
    // file_change_extension(filename, "wav")
