...
static struct {
    ...
    int is_collapsed;
    new_sidebar_extra_display info_to_display;
    int game_speed;
    struct {
        int percentage;
        int amount;
...
static new_sidebar_extra_display calculate_displayable_info(new_sidebar_extra_display info_to_display, int available_height)
{
    if (data.is_collapsed || !config_get(CONFIG_UI_SIDEBAR_INFO) || info_to_display == NEW_SIDEBAR_EXTRA_DISPLAY_NONE) {
        return NEW_SIDEBAR_EXTRA_DISPLAY_NONE;
    }
    new_sidebar_extra_display result = NEW_SIDEBAR_EXTRA_DISPLAY_NONE;
    if (available_height >= EXTRA_INFO_HEIGHT_GAME_SPEED) {
        if (info_to_display & NEW_SIDEBAR_EXTRA_DISPLAY_GAME_SPEED) {
    ...
    if (available_height >= EXTRA_INFO_HEIGHT_UNEMPLOYMENT) {
        if (info_to_display & NEW_SIDEBAR_EXTRA_DISPLAY_UNEMPLOYMENT) {
            available_height -= EXTRA_INFO_HEIGHT_UNEMPLOYMENT;
            result |= NEW_SIDEBAR_EXTRA_DISPLAY_UNEMPLOYMENT;
        }
    } else {
        return result;
    }
    if (available_height >= EXTRA_INFO_HEIGHT_INVASIONS) {
        if (info_to_display & NEW_SIDEBAR_EXTRA_DISPLAY_INVASIONS) {
            available_height -= EXTRA_INFO_HEIGHT_INVASIONS;
            result |= NEW_SIDEBAR_EXTRA_DISPLAY_INVASIONS;
        }
    } else {
        return result;
    }
    if (available_height >= EXTRA_INFO_HEIGHT_GODS) {
        if (info_to_display & NEW_SIDEBAR_EXTRA_DISPLAY_GODS) {
    ...
    if (available_height >= EXTRA_INFO_HEIGHT_REQUESTS_MIN) {
        if (info_to_display & NEW_SIDEBAR_EXTRA_DISPLAY_REQUESTS) {
            available_height -= EXTRA_INFO_HEIGHT_REQUESTS_MIN;
            result |= NEW_SIDEBAR_EXTRA_DISPLAY_REQUESTS;
        }
    }
    if (available_height >= EXTRA_INFO_HEIGHT_RATINGS) {
        if (info_to_display & NEW_SIDEBAR_EXTRA_DISPLAY_RATINGS) {
            available_height -= EXTRA_INFO_HEIGHT_RATINGS;
            result |= NEW_SIDEBAR_EXTRA_DISPLAY_RATINGS;
        }
    } else {
        return result;
    }
...
{
    if (data.info_to_display == NEW_SIDEBAR_EXTRA_DISPLAY_NONE) {
        return 0;
    }
    int height = 0;
    if (data.info_to_display & NEW_SIDEBAR_EXTRA_DISPLAY_GAME_SPEED) {
        height += EXTRA_INFO_HEIGHT_GAME_SPEED;
    }
    if (data.info_to_display & NEW_SIDEBAR_EXTRA_DISPLAY_UNEMPLOYMENT) {
    ...
    if (data.info_to_display & NEW_SIDEBAR_EXTRA_DISPLAY_GODS) {
        height += EXTRA_INFO_HEIGHT_GODS;
    }
    if (data.info_to_display & NEW_SIDEBAR_EXTRA_DISPLAY_REQUESTS) {
    ...
    if (data.info_to_display & NEW_SIDEBAR_EXTRA_DISPLAY_RATINGS) {
...
int new_sidebar_extra_draw_background(int x_offset, int y_offset, int width, int available_height,
...
static void draw_extra_info_buttons(void)
{
    ...
    if (data.info_to_display & NEW_SIDEBAR_EXTRA_DISPLAY_REQUESTS && data.active_requests) {
        for (unsigned int i = 0; i < data.visible_requests; i++) {
...
{
    if (!new_sidebar_extra_is_information_displayed(NEW_SIDEBAR_EXTRA_DISPLAY_RATINGS)) {
        return 0;
    }
