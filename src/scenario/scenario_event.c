...
void scenario_event_load_state(buffer *buf, int version, scenario_event_t *event)
{
    ...
    if (expected_id != event->id) {
        log_error("Scenario events in loaded file out of order!", 0, 0);
    }
    ...
    if (!array_init(event->conditions, SCENARIO_CONDITIONS_ARRAY_SIZE_STEP, 0, 0) ||
        !array_expand(event->conditions, conditions_count)) {
        log_error("Unable to create conditions array. The game will now crash.", 0, 0);
    }
    if (!array_init(event->actions, SCENARIO_ACTIONS_ARRAY_SIZE_STEP, 0, 0) ||
...
void initialize_new_scenario_event(scenario_event_t *event, int position)
{
    event->id = position;
}
...
{
    return (event->repeat_months_min > 0)
        && (event->repeat_months_max >= event->repeat_months_min)
        && ((event->execution_count < event->max_number_of_repeats)
            || (event->max_number_of_repeats <= 0));
}
