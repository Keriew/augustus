...
custom_media_t *custom_media_create_blank(void)
{
    ...
    array_new_item(custom_media, 0, entry);
    if (!entry) {
        return 0;
    }
...
void custom_media_save_state(buffer *buffer)
{
    ...
    array_foreach(custom_media, entry) {
        buffer_write_i32(buffer, entry->id);
        buffer_write_i32(buffer, entry->type);
        buffer_write_i32(buffer, entry->filename->id);
        buffer_write_i16(buffer, entry->link_type);
        buffer_write_i32(buffer, entry->link_id);
    }
}
